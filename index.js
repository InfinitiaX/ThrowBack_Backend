// index.js - Version complètement corrigée
...
const extractUser = async (req, res, next) => {
  try {
    ...
    } else if (req.cookies && req.cookies.token) { // ✅ Vérification ajoutée
...
app.use((req, res, next) => {
  ...
  if (req.user) {
    res.locals.isAdmin = req.user.roles && req.user.roles.some(role => 
      ['admin', 'superadmin'].includes(role.libelle_role || role)
    );
    res.locals.isSuperAdmin = req.user.roles && req.user.roles.some(role => 
      (role.libelle_role || role) === 'superadmin'
    );
  } else {
...
try {
  ...
  console.log("⚠️  Routes web auth non disponibles:", error.message);
...
try {
  ...
  console.log("⚠️  Routes d'administration non disponibles:", error.message);
  ...
  app.get('/admin', (req, res) => {
    ...
    res.json({
      message: 'Interface d\'administration - En construction',
      user: req.user
    });
  });
}
