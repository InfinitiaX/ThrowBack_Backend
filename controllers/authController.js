...
const verifyEmail = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      return res.redirect(`${process.env.FRONTEND_URL || 'https://testdevinfinitiax.fr'}/login?error=invalid_link&message=Invalid verification link`);
    }
    ...
    if (user.statut_verification) {
      ...
      return res.redirect(`${process.env.FRONTEND_URL || 'https://testdevinfinitiax.fr'}/login?verified=true&message=Your account is already verified. You can now sign in.`);
    }
    ...
    if (!tokenDoc) {
      ...
      return res.redirect(`${process.env.FRONTEND_URL || 'https://testdevinfinitiax.fr'}/login?error=expired_link&message=Verification link expired`);
    }
    ...
    res.redirect(`${process.env.FRONTEND_URL || 'https://testdevinfinitiax.fr'}/login?verified=true&message=Email verified successfully. You can now sign in.`);
  } catch (error) {
    console.error(" Email verification error:", error);
    res.redirect(`${process.env.FRONTEND_URL || 'https://testdevinfinitiax.fr'}/login?error=server_error&message=An error occurred during verification`);
  }
};
...
const verifyPasswordReset = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      return res.redirect(`${process.env.FRONTEND_URL || 'https://testdevinfinitiax.fr'}/forgot-password?error=invalid_token&message=Invalid or expired token`);
    }
    
    console.log(" Valid token, redirecting to reset form");
    
    // Valid token, redirect to reset form (SANS ESPACE DANS L'URL)
    res.redirect(`${process.env.FRONTEND_URL || 'https://testdevinfinitiax.fr'}/reset-password?token=${token}&message=Valid token, you can now set your new password`);
  } catch (error) {
    console.error(" Password reset token verification error:", error);
    res.redirect(`${process.env.FRONTEND_URL || 'https://testdevinfinitiax.fr'}/forgot-password?error=server_error&message=An error occurred`);
  }
};
